...
def draw_square(event):
    global x,y,size_num, outline, color_num
    canvas.create_rectangle((x,y,event.x,event.y),outline=outline, fill=color_num, width = size_num)
    x,y = event.x,event.y

def draw_circle(event):
    global x,y,size_num,outline,color_num
    canvas.create_oval((x,y,event.x,event.y),outline=outline, fill=color_num, width=size_num)
    x,y=event.x,event.y
